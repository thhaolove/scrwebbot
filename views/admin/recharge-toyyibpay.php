...
if (isset($_POST['SaveSettings'])) {
    ...
    $CMSNT->insert("logs", [
        ...
        'device'        => getUserAgent(),
        'createdate'    => gettime(),
        'action'        => __('Cấu hình nạp tiền Toyyibpay')
    ]);
    ...
    $my_text = str_replace('{action}', __('Cấu hình nạp tiền Toyyibpay'), $my_text);
...
$urlDatatable = pagination(base_url_admin("recharge-toyyibpay&limit=$limit&shortByDate=$shortByDate&create_gettime=$createdate&trans_id=$trans_id&amount=$amount&status=$status&user_id=$user_id&username=$username&BillCode=$BillCode&billName=$billName&"), $from, $totalDatatable, $limit);
...
$total_yesterday = intval($CMSNT->get_row("SELECT SUM(amount) FROM payment_toyyibpay WHERE  `status` = 1 AND `create_gettime` LIKE '%".$yesterday."%' ")['SUM(amount)']);
$total_today = $CMSNT->get_row("SELECT SUM(amount) FROM payment_toyyibpay WHERE  `status` = 1 AND `create_gettime` LIKE '%".$currentDate."%' ")['SUM(amount)'];
$total_all_time = $CMSNT->get_row("SELECT SUM(amount) FROM payment_toyyibpay WHERE  `status` = 1 ")['SUM(amount)'];
...
<div class="main-content app-content">
    <div class="container-fluid">
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <h1 class="page-title fw-semibold fs-18 mb-0">Phương thức thanh toán Toyyibpay Malaysia</h1>
                            ...
                            <div class="row">
                                <div class="col-lg-12 col-xl-6">
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <select class="form-control" name="toyyibpay_status">
                                                <option <?=$CMSNT->site('toyyibpay_status') == 1 ? 'selected' : '';?>
                                                    value="1">
                                                    ON
                                                </option>
                                                <option <?=$CMSNT->site('toyyibpay_status') == 0 ? 'selected' : '';?>
                                                    value="0">
                                                    OFF
                                                </option>
                                            </select>
                                    ...
                                    <div class="row mb-4">
                                        <label class="col-sm-4 col-form-label" for="example-hf-email">User Secret
                                            Key</label>
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control"
                                                value="<?=$CMSNT->site('toyyibpay_userSecretKey');?>"
                                                name="toyyibpay_userSecretKey">
                                        </div>
                                    </div>
                                    <div class="row mb-4">
                                        <label class="col-sm-4 col-form-label" for="example-hf-email">Category
                                            Code</label>
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control"
                                                value="<?=$CMSNT->site('toyyibpay_categoryCode');?>"
                                                name="toyyibpay_categoryCode">
                                        </div>
                                    </div>
                                </div>
                                <div class="col-lg-12 col-xl-6">
                                    <div class="row mb-4">
                                        <label class="col-sm-7 col-form-label" for="example-hf-email">1
                                            <?=$CMSNT->get_row(" SELECT `code` FROM `currencies` WHERE `display` = 1 AND `default_currency` = 1")['code'];?>
                                            is equal to how many RM</label>
                                        <div class="col-sm-5">
                                            <div class="input-group">
                                                <input type="text" class="form-control"
                                                    value="<?=$CMSNT->site('toyyibpay_rate');?>" name="toyyibpay_rate"
                                                    placeholder="5258">
                                    ...
                                    <div class="row mb-4">
                                        <label class="col-sm-7 col-form-label"
                                            for="example-hf-email"><?=__('Minimum deposit amount');?></label>
                                        <div class="col-sm-5">
                                            <div class="input-group">
                                                <input type="text" class="form-control"
                                                    value="<?=$CMSNT->site('toyyibpay_min');?>" name="toyyibpay_min"
                                                    placeholder="1 = RM1">
                                                <span class="input-group-text">
                                                    RM
                                                </span>
                                    ...
                                    <div class="row mb-4">
                                        <label class="col-sm-6 col-form-label"
                                            for="example-hf-email"><?=__('FPX fee option');?></label>
                                        <div class="col-sm-6">
                                            <select class="form-control" name="toyyibpay_billChargeToCustomer">
                                                <option
                                                    <?=$CMSNT->site('toyyibpay_billChargeToCustomer') == '0' ? 'selected' : '';?>
                                                    value="0">Your customers pay a fee
                                                </option>
                                                <option
                                                    <?=$CMSNT->site('toyyibpay_billChargeToCustomer') == '' ? 'selected' : '';?>
                                                    value="">You pay fees
                                                </option>
                                            </select>
                                        </div>
                                ...
                                <div class="col-lg-12 col-xl-12">
                                    <div class="row mb-4">
                                        <label class="col-sm-6 col-form-label"
                                            for="example-hf-email"><?=__('Toyyibpay Note');?></label>
                                        <div class="col-sm-12">
                                            <textarea id="toyyibpay_notice"
                                                name="toyyibpay_notice"><?=$CMSNT->site('toyyibpay_notice');?></textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?=format_cash($total_all_time);?> RM</p>
                        ...
                        <div class="card custom-card">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?=format_cash($CMSNT->get_row("SELECT SUM(amount) FROM payment_toyyibpay WHERE  `status` = 1 AND MONTH(create_gettime) = '$currentMonth' AND YEAR(create_gettime) = '$currentYear' ")['SUM(amount)']);?>
                                            RM
                                        </p>
                        ...
                        <div class="card custom-card">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?=format_cash($CMSNT->get_row("SELECT SUM(amount) FROM payment_toyyibpay WHERE  `status` = 1 AND YEAR(create_gettime) = $currentYear AND WEEK(create_gettime, 1) = $currentWeek ")['SUM(amount)']);?>
                                            RM
                                        </p>
      ...
      echo '<span class="fs-12 text-danger ms-2"><i class="ti ti-trending-down me-1 d-inline-block"></i>' . round(abs($revenueGrowth), 2) . '% </span>';
            ...
            <div class="col-xl-7">
              ...
              ?>
                  ...
                  for ($day = 1; $day <= $numOfDays; $day++) {
                      $date = "$year-$month-$day";
                      $row = $CMSNT->get_row("SELECT SUM(amount) FROM payment_toyyibpay WHERE DATE(create_gettime) = '$date' AND  `status` = 1 ");
                ...
                <div class="card custom-card">
                    <div class="card-header justify-content-between">
                        <div class="card-title">
                            LỊCH SỬ NẠP TIỀN TOYYIBPAY TỰ ĐỘNG
                        </div>
                    </div>
                    <div class="card-body">
                        <form action="" class="align-items-center mb-3" name="formSearch" method="GET">
                            <div class="row row-cols-lg-auto g-3 mb-3">
                                ...
                                <div class="col-md-3 col-6">
                                    <input class="form-control form-control-sm" value="<?=$BillCode;?>" name="BillCode"
                                        placeholder="<?=__('Search bill code');?>">
                                </div>
                                <div class="col-md-3 col-6">
                                    <input class="form-control form-control-sm" value="<?=$billName;?>" name="billName"
                                        placeholder="<?=__('Search bill name');?>">
                                </div>
                                <div class="col-md-3 col-6">
                                ...
                                <div class="col-12">
                                    ...
                                    <a class="btn btn-sm btn-danger"
                                        href="<?=base_url_admin('recharge-toyyibpay');?>"><i class="fa fa-trash"></i>
                                        <?=__('Clear filter');?>
                                    </a>
                                </div>
                            </div>
                                    ...
                                    <tr>
                                        <td colspan="7">
                                            <div class="float-right">
                                                <?=__('Paid:');?>
                                                <strong
                                                    style="color:red;"><?=format_cash($CMSNT->get_row(" SELECT SUM(`amount`) FROM `payment_toyyibpay` WHERE $where AND `status` = 1 ")['SUM(`amount`)']);?>
                                                    RM</strong>
...
<script>
CKEDITOR.replace("toyyibpay_notice");
</script>