...
if (isset($_POST['SaveSettings'])) {
    ...
    $CMSNT->insert("logs", [
        ...
        'device'        => getUserAgent(),
        'createdate'    => gettime(),
        'action'        => __('Cấu hình nạp tiền LemPay')
    ]);

    // Xử lý upload icon
    if (!empty($_FILES['lempay_icon_file']['name'])) {
        $allowed_types = ['image/png', 'image/jpeg', 'image/jpg', 'image/gif', 'image/webp', 'image/svg+xml'];
        $file_type = $_FILES['lempay_icon_file']['type'];
        if (in_array($file_type, $allowed_types)) {
            $ext = pathinfo($_FILES['lempay_icon_file']['name'], PATHINFO_EXTENSION);
            $new_filename = 'logo-lempay-' . time() . '.' . $ext;
            $upload_path = __DIR__ . '/../../mod/img/' . $new_filename;
            if (move_uploaded_file($_FILES['lempay_icon_file']['tmp_name'], $upload_path)) {
                $_POST['lempay_icon'] = 'mod/img/' . $new_filename;
            }
        }
    }
    ...
    $my_text = str_replace('{action}', __('Cấu hình nạp tiền LemPay'), $my_text);
...
$listDatatable = $CMSNT->get_list(" SELECT * FROM `payment_lempay` WHERE $where ORDER BY `id` DESC LIMIT $from,$limit ");
$totalDatatable = $CMSNT->num_rows(" SELECT * FROM `payment_lempay` WHERE $where ORDER BY id DESC ");
$urlDatatable = pagination(base_url_admin("recharge-lempay&limit=$limit&shortByDate=$shortByDate&created_at=$createdate&trans_id=$trans_id&amount=$amount&user_id=$user_id&username=$username&status=$status&type=$type&"), $from, $totalDatatable, $limit);
...
$total_yesterday = intval($CMSNT->get_row("SELECT SUM(price) FROM payment_lempay WHERE `status` = 1 AND  `created_at` LIKE '%" . $yesterday . "%' ")['SUM(price)']);
$total_today = $CMSNT->get_row("SELECT SUM(price) FROM payment_lempay WHERE `status` = 1 AND `created_at` LIKE '%" . $currentDate . "%' ")['SUM(price)'];
$total_all_time = $CMSNT->get_row("SELECT SUM(price) FROM payment_lempay WHERE `status` = 1 ")['SUM(price)'];

$checkKey = checkAddonLicense($CMSNT->site('lempay_license'), 'SHOPCLONE7_GATEWAY_LEMPAY');
...
<div class="main-content app-content">
    <div class="container-fluid">
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <h1 class="page-title fw-semibold fs-18 mb-0">Phương thức thanh toán LemPay</h1>
        </div>
        <div class="row">
            <div class="col-xl-12">
                                ...
                                <?php if ($checkKey['status'] != true): ?>
                                    <div class="col-lg-12 col-xl-12">
                                        <div class="row mb-4">
                                            ...
                                            <div class="col-sm-5">
                                                <select class="form-control" name="lempay_status">
                                                    <option <?= $CMSNT->site('lempay_status') == 1 ? 'selected' : ''; ?> value="1">ON</option>
                                                    <option <?= $CMSNT->site('lempay_status') == 0 ? 'selected' : ''; ?> value="0">OFF</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="row mb-4">
                                            ...
                                            <div class="col-sm-5">
                                                <input type="text" class="form-control" placeholder="921abf4dbff01xxxxxf3c562c356c769" value="<?= $CMSNT->site('lempay_license'); ?>" name="lempay_license">
                                            </div>
                                            <div style="margin-top: 10px; padding: 10px; background-color: #fff3cd; border: 1px solid #ffeeba; border-radius: 4px; color: #856404;">
                                                <strong>Chú ý:</strong> Bạn cần phải mua giấy phép kích hoạt <a target="_blank" style="color: #007bff;" href="https://client.cmsnt.co/cart.php?a=add&pid=103">Addon</a> trước khi sử dụng. Truy cập Admin/Cài Đặt/Addons để mua giấy phép hoặc nhấn vào <a target="_blank" style="color: #007bff;" href="https://client.cmsnt.co/cart.php?a=add&pid=103">đây</a>.
                                            </div>
                                        </div>
                                    </div>
                                <?php else: ?>
                                    <div class="col-lg-12 col-xl-6">
                                        ...
                                        <div class="row mb-4">
                                            ...
                                            <div class="col-sm-7">
                                                <input type="text" class="form-control" value="<?= $CMSNT->site('lempay_name') ?: 'LemPay (AliPay, WeChat, USDT)'; ?>" name="lempay_name" placeholder="LemPay (AliPay, WeChat, USDT)">
                                        ...
                                        <div class="row mb-4">
                                            ...
                                            <div class="col-sm-7">
                                                <div class="d-flex align-items-center mb-2">
                                                    <?php if ($CMSNT->site('lempay_icon')): ?>
                                                        <img src="<?= base_url($CMSNT->site('lempay_icon')); ?>" width="40" height="40" class="me-2 rounded" style="object-fit: contain;">
                                                    <?php endif; ?>
                                                    <input type="file" class="form-control form-control-sm" name="lempay_icon_file" accept="image/*">
                                                </div>
                                                <small class="text-muted">Upload ảnh logo mới (PNG, JPG, WEBP, SVG)</small>
                                            </div>
                                        </div>
                                        <div class="row mb-4">
                                            <label class="col-sm-5 col-form-label">PID (Partner ID)</label>
                                            <div class="col-sm-7">
                                                <input type="text" class="form-control" value="<?= $CMSNT->site('lempay_pid'); ?>" name="lempay_pid" placeholder="1001">
                                            </div>
                                        </div>
                                        <div class="row mb-4">
                                            <label class="col-sm-5 col-form-label">Merchant Key</label>
                                            <div class="col-sm-7">
                                                <input type="text" class="form-control" value="<?= $CMSNT->site('lempay_key'); ?>" name="lempay_key" placeholder="xxxxxxxxxxxxxx">
                                            </div>
                                        </div>
                                        <div class="row mb-4">
                                            <label class="col-sm-5 col-form-label">API URL</label>
                                            <div class="col-sm-7">
                                                <input type="text" class="form-control" value="<?= $CMSNT->site('lempay_api_url') ?: 'https://a119a.lempay.com'; ?>" name="lempay_api_url">
                                                <small>Mặc định: https://a119a.lempay.com</small>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-lg-12 col-xl-6">
                                        <div class="row mb-4">
                                            <label class="col-sm-5 col-form-label">Min (CNY)</label>
                                            <div class="col-sm-7">
                                                <input type="text" class="form-control" value="<?= $CMSNT->site('lempay_min'); ?>" name="lempay_min">
                                            </div>
                                        </div>
                                        <div class="row mb-4">
                                            <label class="col-sm-5 col-form-label">Max (CNY)</label>
                                            <div class="col-sm-7">
                                                <input type="text" class="form-control" value="<?= $CMSNT->site('lempay_max'); ?>" name="lempay_max">
                                            </div>
                                        </div>
                                        <div class="row mb-4">
                                            <label class="col-sm-5 col-form-label"><?= __('Rate mặc định (1 CNY =)'); ?></label>
                                            <div class="col-sm-7">
                                                <div class="input-group">
                                                    <input type="text" class="form-control" value="<?= $CMSNT->site('lempay_rate'); ?>" name="lempay_rate">
                                                    <span class="input-group-text"><?= $CMSNT->get_row("SELECT `code` FROM `currencies` WHERE `display` = 1 AND `default_currency` = 1")['code']; ?></span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <!-- Tỉ giá riêng cho từng phương thức -->
                                    <div class="col-lg-12">
                                        <hr>
                                        <h5 class="mb-3">Tỉ giá riêng cho từng phương thức thanh toán</h5>
                                    </div>
                                    <div class="col-lg-12 col-xl-4">
                                        <div class="row mb-4">
                                            <label class="col-sm-5 col-form-label">
                                                <i class="fab fa-alipay text-primary"></i> Alipay Rate
                                            </label>
                                            <div class="col-sm-7">
                                                <input type="text" class="form-control" value="<?= $CMSNT->site('lempay_rate_alipay') ?: $CMSNT->site('lempay_rate'); ?>" name="lempay_rate_alipay" placeholder="Để trống = dùng rate mặc định">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-lg-12 col-xl-4">
                                        <div class="row mb-4">
                                            <label class="col-sm-5 col-form-label">
                                                <i class="fab fa-weixin text-success"></i> WeChat Rate
                                            </label>
                                            <div class="col-sm-7">
                                                <input type="text" class="form-control" value="<?= $CMSNT->site('lempay_rate_wxpay') ?: $CMSNT->site('lempay_rate'); ?>" name="lempay_rate_wxpay" placeholder="Để trống = dùng rate mặc định">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-lg-12 col-xl-4">
                                        <div class="row mb-4">
                                            <label class="col-sm-5 col-form-label">
                                                <i class="fab fa-bitcoin text-warning"></i> USDT Rate
                                            </label>
                                            <div class="col-sm-7">
                                                <input type="text" class="form-control" value="<?= $CMSNT->site('lempay_rate_usdt') ?: $CMSNT->site('lempay_rate'); ?>" name="lempay_rate_usdt" placeholder="Để trống = dùng rate mặc định">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-lg-12">
                                        ...
                                        <div class="row mb-4">
                                            ...
                                            <div class="col-sm-10">
                                                <textarea id="lempay_notice" name="lempay_notice"><?= $CMSNT->site('lempay_notice'); ?></textarea>
                                            </div>
                                        </div>
                                    </div>
                                <?php endif ?>
                                ...
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?= format_currency($CMSNT->get_row("SELECT SUM(price) FROM payment_lempay WHERE `status` = 1 AND MONTH(created_at) = '$currentMonth' AND YEAR(created_at) = '$currentYear' ")['SUM(price)']); ?>
                                        </p>
                        ...
                        <div class="card custom-card">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?= format_currency($CMSNT->get_row("SELECT SUM(price) FROM payment_lempay WHERE `status` = 1 AND YEAR(created_at) = $currentYear AND WEEK(created_at, 1) = $currentWeek ")['SUM(price)']); ?>
                                        </p>
                                ...
                                const data = {
                                    ...
                                    datasets: [{
                                        ...
                                        data: [
                                            ...
                                            for ($day = 1; $day <= $numOfDays; $day++) {
                                                $date = "$year-$month-$day";
                                                $row = $CMSNT->get_row("SELECT SUM(price) FROM payment_lempay WHERE `status` = 1 AND DATE(created_at) = '$date' ");
            ...
            <div class="col-xl-12">
                <div class="card custom-card">
                    <div class="card-header justify-content-between">
                        <div class="card-title">LỊCH SỬ NẠP TIỀN LEMPAY</div>
                    </div>
                    <div class="card-body">
                        <form action="" class="align-items-center mb-3" name="formSearch" method="GET">
                            <div class="row row-cols-lg-auto g-3 mb-3">
                                ...
                                <div class="col-12">
                                    ...
                                    <a class="btn btn-sm btn-danger" href="<?= base_url_admin('recharge-lempay'); ?>"><i class="fa fa-trash"></i> <?= __('Clear filter'); ?></a>
                                </div>
                            </div>
                                    ...
                                    <?php foreach ($listDatatable as $row): ?>
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                ...
                                                echo $typeIcons[$row['type']] ?? $row['type'];
                                                ?>
                                            </td>
                                            <td class="text-right"><b><?= format_cash($row['amount']); ?></b> CNY</td>
                                    ...
                                    <tr>
                                        <td colspan="7">
                                            <div class="float-right">
                                                <?= __('Paid:'); ?> <strong style="color:red;"><?= format_currency($CMSNT->get_row(" SELECT SUM(`price`) FROM `payment_lempay` WHERE $where AND `status` = 1 ")['SUM(`price`)']); ?></strong>
                                            </div>
                                        </td>
                                    </tr>
                                </tfoot>
...
<script>
    CKEDITOR.replace("lempay_notice");
</script>
