...
$body = [
    'title' => __('Recharge Perfect Money'),
    'desc'   => 'CMSNT Panel',
...
if (isset($_POST['SaveSettings'])) {
    ...
    $CMSNT->insert("logs", [
        ...
        'device'        => getUserAgent(),
        'createdate'    => gettime(),
        'action'        => __('Cấu hình nạp tiền Perfect Money')
    ]);
    ...
    $my_text = str_replace('{action}', __('Cấu hình nạp tiền Perfect Money'), $my_text);
...
$urlDatatable = pagination(base_url_admin("recharge-perfectmoney&limit=$limit&shortByDate=$shortByDate&time=$time&payment_id=$payment_id&amount=$amount&user_id=$user_id&username=$username&"), $from, $totalDatatable, $limit);
...
$total_yesterday = intval($CMSNT->get_row("SELECT SUM(price) FROM payment_pm WHERE `status` = 1 AND `create_date` LIKE '%".$yesterday."%' ")['SUM(price)']);
$total_today = $CMSNT->get_row("SELECT SUM(price) FROM payment_pm WHERE `status` = 1 AND `create_date` LIKE '%".$currentDate."%' ")['SUM(price)'];
$total_all_time = $CMSNT->get_row("SELECT SUM(price) FROM payment_pm WHERE `status` = 1 ")['SUM(price)'];
...
<div class="main-content app-content">
    <div class="container-fluid">
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <h1 class="page-title fw-semibold fs-18 mb-0"><i class="fa-solid fa-receipt"></i> Phương thức thanh toán Perfect Money</h1>
        </div>
        <div class="row">
            <div class="col-xl-12">
                            ...
                            <div class="row">
                                <div class="col-lg-12 col-xl-6">
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <select class="form-control" name="perfectmoney_status">
                                                <option <?=$CMSNT->site('perfectmoney_status') == 1 ? 'selected' : '';?>
                                                    value="1">ON
                                                </option>
                                                <option <?=$CMSNT->site('perfectmoney_status') == 0 ? 'selected' : '';?>
                                                    value="0">OFF
                                                </option>
                                            </select>
                                        </div>
                                        ...
                                        <div class="col-sm-8">
                                            <div class="input-group">
                                                <input type="text" class="form-control"
                                                    value="<?=$CMSNT->site('perfectmoney_rate');?>"
                                                    name="perfectmoney_rate" placeholder="VD: 23000">
                                    ...
                                    <div class="row mb-4">
                                        <label class="col-sm-4 col-form-label"
                                            for="example-hf-email"><?=__('Đơn vị tiền tệ');?></label>
                                        <div class="col-sm-8">
                                            <div class="input-group">
                                                <input type="text" class="form-control"
                                                    value="<?=$CMSNT->site('perfectmoney_units');?>"
                                                    name="perfectmoney_units" placeholder="VD: USD">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-lg-12 col-xl-6">
                                    <div class="row mb-4">
                                        <label class="col-sm-5 col-form-label"
                                            for="example-hf-email"><?=__('Mã tài khoản Perfect Money');?></label>
                                        <div class="col-sm-7">
                                            <input type="text" class="form-control"
                                                value="<?=$CMSNT->site('perfectmoney_account');?>"
                                                name="perfectmoney_account" placeholder="VD: U36599423">
                                        </div>
                                    </div>
                                    <div class="row mb-4">
                                        <label class="col-sm-5 col-form-label"
                                            for="example-hf-email"><?=__('Mật khẩu Thay thế (Alternate Passphrase)');?></label>
                                        <div class="col-sm-7">
                                            <input type="text" class="form-control"
                                                value="<?=$CMSNT->site('perfectmoney_pass');?>" name="perfectmoney_pass"
                                                placeholder="<?=__('Nhập mật khẩu thay thế');?>">
                                        </div>
                                    </div>
                                </div>
                                <div class="col-lg-12 col-xl-12">
                                    <div class="row mb-4">
                                        <label class="col-sm-6 col-form-label"
                                            for="example-hf-email"><?=__('Perfect Money Note');?></label>
                                        <div class="col-sm-12">
                        ...
                        <div class="card custom-card">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?=format_currency($CMSNT->get_row("SELECT SUM(price) FROM payment_pm WHERE `status` = 1 AND MONTH(create_date) = '$currentMonth' AND YEAR(create_date) = '$currentYear' ")['SUM(price)']);?>
                                        </p>
                        ...
                        <div class="card custom-card">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?=format_currency($CMSNT->get_row("SELECT SUM(price) FROM payment_pm WHERE `status` = 1 AND YEAR(create_date) = $currentYear AND WEEK(create_date, 1) = $currentWeek ")['SUM(price)']);?>
                                        </p>
      ...
      echo '<span class="fs-12 text-danger ms-2"><i class="ti ti-trending-down me-1 d-inline-block"></i>' . round(abs($revenueGrowth), 2) . '% </span>';
            ...
            <div class="col-xl-7">
              ...
              ?>
                  ...
                  for ($day = 1; $day <= $numOfDays; $day++) {
                      $date = "$year-$month-$day";
                      $row = $CMSNT->get_row("SELECT SUM(price) FROM payment_pm WHERE `status` = 1 AND DATE(create_date) = '$date' ");
                ...
                <div class="card custom-card">
                    <div class="card-header justify-content-between">
                        <div class="card-title">
                            LỊCH SỬ NẠP TIỀN PERFECT MONEY TỰ ĐỘNG
                        </div>
                    </div>
                    <div class="card-body">
                        <form action="" class="align-items-center mb-3" name="formSearch" method="GET">
                            <div class="row row-cols-lg-auto g-3 mb-3">
                                ...
                                <div class="col-md-3 col-6">
                                    <input class="form-control form-control-sm" value="<?=$payment_id;?>"
                                        name="payment_id" placeholder="<?=__('Mã giao dịch');?>">
                                </div>
                                <div class="col-md-3 col-6">
                                ...
                                <div class="col-12">
                                    ...
                                    <a class="btn btn-sm btn-danger"
                                        href="<?=base_url_admin('recharge-perfectmoney');?>"><i class="fa fa-trash"></i>
                                        <?=__('Clear filter');?>
                                    </a>
                                </div>
                            </div>
                            ...
                            <table class="table text-nowrap table-striped table-hover table-bordered">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td style="text-align: right;"><span
                                                class="badge bg-danger-gradient"><?=format_currency($row2['price']);?>
                                        </td>
                                    ...
                                    <tr>
                                        <td colspan="7">
                                            <div class="float-right">
                                                <?=__('Paid:');?>
                                                <strong
                                                    style="color:red;"><?=format_currency($CMSNT->get_row(" SELECT SUM(`price`) FROM `payment_pm` WHERE $where   ")['SUM(`price`)']);?></strong>
