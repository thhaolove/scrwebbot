...
$body = [
    'title' => __('Recharge PaymentPoint'),
    'desc'   => 'CMSNT Panel',
...
if (isset($_POST['SaveSettings'])) {
    ...
    $CMSNT->insert("logs", [
        ...
        'device'        => getUserAgent(),
        'createdate'    => gettime(),
        'action'        => __('Cấu hình nạp tiền PaymentPoint')
    ]);
    ...
    $my_text = str_replace('{action}', __('Cấu hình nạp tiền PaymentPoint'), $my_text);
...
$listDatatable = $CMSNT->get_list(" SELECT * FROM `payment_paymentpoint` WHERE $where ORDER BY `id` DESC LIMIT $from,$limit ");
$totalDatatable = $CMSNT->num_rows(" SELECT * FROM `payment_paymentpoint` WHERE $where ORDER BY id DESC ");
$urlDatatable = pagination(base_url_admin("recharge-paymentpoint&limit=$limit&shortByDate=$shortByDate&created_at=$createdate&trans_id=$trans_id&amount=$amount&user_id=$user_id&username=$username&status=$status&"), $from, $totalDatatable, $limit);
...
$total_yesterday = intval($CMSNT->get_row("SELECT SUM(price) FROM payment_paymentpoint WHERE `status` = 1 AND  `created_at` LIKE '%".$yesterday."%' ")['SUM(price)']);
$total_today = $CMSNT->get_row("SELECT SUM(price) FROM payment_paymentpoint WHERE `status` = 1 AND `created_at` LIKE '%".$currentDate."%' ")['SUM(price)'];
$total_all_time = $CMSNT->get_row("SELECT SUM(price) FROM payment_paymentpoint WHERE `status` = 1 ")['SUM(price)'];


$checkKey = checkAddonLicense($CMSNT->site('paymentpoint_license'), 'SHOPCLONE7_GATEWAY_PAYMENTPOINT');
...
<div class="main-content app-content">
    <div class="container-fluid">
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <h1 class="page-title fw-semibold fs-18 mb-0">Phương thức thanh toán PaymentPoint</h1>
        </div>
        <div class="row">
            <div class="col-xl-12">
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-12 col-xl-12">
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-5">
                                            <select class="form-control" name="paymentpoint_status">
                                                <option <?=$CMSNT->site('paymentpoint_status') == 1 ? 'selected' : '';?>
                                                    value="1">ON
                                                </option>
                                                <option <?=$CMSNT->site('paymentpoint_status') == 0 ? 'selected' : '';?>
                                                    value="0">
                                                    OFF
                                                </option>
                                            </select>
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-5">
                                            <input type="text" class="form-control"
                                                ...
                                                value="<?=$CMSNT->site('paymentpoint_license');?>" name="paymentpoint_license">
                                        </div>
                                        <div
                                            ...
                                            <strong>Chú ý:</strong> Bạn cần phải mua giấy phép kích hoạt <a
                                                ...
                                                href="https://client.cmsnt.co/cart.php?a=add&pid=97">Addon</a> trước
                                            khi sử dụng. Truy cập Admin/Cài Đặt/Addons để mua giấy phép hoặc nhấn vào <a
                                                target="_blank" style="color: #007bff;"
                                                href="https://client.cmsnt.co/cart.php?a=add&pid=97">đây</a>.
                                        </div>
                                    </div>
                                </div>
                                <?php else:?>
                                <div class="col-lg-12 col-xl-6">
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-5">
                                            <input type="text" class="form-control"
                                                value="<?=$CMSNT->site('paymentpoint_currency_code');?>"
                                                name="paymentpoint_currency_code">
                                        </div>
                                    </div>
                                    <div class="row mb-4">
                                        <label class="col-sm-7 col-form-label"
                                            for="example-hf-email"><?=__('1 '.$CMSNT->site('paymentpoint_currency_code').' =');?></label>
                                        <div class="col-sm-5">
                                            <div class="input-group">
                                                <input type="text" class="form-control"
                                                    value="<?=$CMSNT->site('paymentpoint_rate');?>" name="paymentpoint_rate"
                                                    placeholder="">
                                ...
                                <div class="col-lg-12 col-xl-6">
                                    <div class="row mb-4">
                                        <label class="col-sm-4 col-form-label" for="example-hf-email">Business ID</label>
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control"
                                                value="<?=$CMSNT->site('paymentpoint_business_id');?>"
                                                name="paymentpoint_business_id">
                                        </div>
                                    </div>
                                    <div class="row mb-4">
                                        <label class="col-sm-4 col-form-label" for="example-hf-email">API Secret</label>
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control"
                                                value="<?=$CMSNT->site('paymentpoint_api_secret');?>" name="paymentpoint_api_secret">
                                            <small>Bearer Token từ PaymentPoint</small>
                                        </div>
                                    </div>
                                    <div class="row mb-4">
                                        <label class="col-sm-4 col-form-label" for="example-hf-email">API Key</label>
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control"
                                                value="<?=$CMSNT->site('paymentpoint_api_key');?>" name="paymentpoint_api_key">
                                            <small>API Key từ PaymentPoint</small>
                                        </div>
                                    </div>
                                    <div class="row mb-4">
                                        <label class="col-sm-4 col-form-label" for="example-hf-email">Bank Codes</label>
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control"
                                                value="<?=$CMSNT->site('paymentpoint_bank_codes');?>" name="paymentpoint_bank_codes">
                                            <small>Mã ngân hàng, cách nhau bởi dấu phẩy. VD: 20946,20897 (PALMPAY: 20946, OPAY: 20897)</small>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-lg-12 col-xl-6">
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control"
                                                value="<?=$CMSNT->site('paymentpoint_min');?>" name="paymentpoint_min">
                                        </div>
                                    </div>
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control"
                                                value="<?=$CMSNT->site('paymentpoint_max');?>" name="paymentpoint_max">
                                        </div>
                                    </div>
                                </div>
                                <div class="col-lg-12 col-xl-12">
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-12">
                                            <textarea id="paymentpoint_notice"
                                                name="paymentpoint_notice"><?=$CMSNT->site('paymentpoint_notice');?></textarea>
                                        </div>
                                    </div>
                                </div>
                                <?php endif?>
                        ...
                        <div class="card custom-card">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?=format_currency($CMSNT->get_row("SELECT SUM(price) FROM payment_paymentpoint WHERE `status` = 1 AND MONTH(created_at) = '$currentMonth' AND YEAR(created_at) = '$currentYear' ")['SUM(price)']);?>
                                        </p>
                        ...
                        <div class="card custom-card">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?=format_currency($CMSNT->get_row("SELECT SUM(price) FROM payment_paymentpoint WHERE `status` = 1 AND YEAR(created_at) = $currentYear AND WEEK(created_at, 1) = $currentWeek ")['SUM(price)']);?>
                                        </p>
      ...
      echo '<span class="fs-12 text-danger ms-2"><i class="ti ti-trending-down me-1 d-inline-block"></i>' . round(abs($revenueGrowth), 2) . '% </span>';
            ...
            <div class="col-xl-7">
              ...
              ?>
                  ...
                  for ($day = 1; $day <= $numOfDays; $day++) {
                      $date = "$year-$month-$day";
                      $row = $CMSNT->get_row("SELECT SUM(price) FROM payment_paymentpoint WHERE `status` = 1 AND DATE(created_at) = '$date' ");
                            ...
                            <div class="row row-cols-lg-auto g-3 mb-3">
                                ...
                                <div class="col-md-3 col-6">
                                    <select name="status" class="form-control form-control-sm">
                                        ...
                                        <option <?=$status == 1 ? 'selected' : '';?> value="1">Waiting
                                        </option>
                                        <option <?=$status == 2 ? 'selected' : '';?> value="2">Completed
                                        </option>
                                        <option <?=$status == 3 ? 'selected' : '';?> value="3">Failed
                                        </option>
                                    </select>
                                </div>
                                ...
                                <div class="col-12">
                                    ...
                                    <a class="btn btn-sm btn-danger" href="<?=base_url_admin('recharge-paymentpoint');?>"><i
                                            class="fa fa-trash"></i>
                                        <?=__('Clear filter');?>
                                    </a>
                                </div>
                                    ...
                                    <tr>
                                        <td colspan="8">
                                            <div class="float-right">
                                                <?=__('Paid:');?>
                                                <strong
                                                    style="color:red;"><?=format_currency($CMSNT->get_row(" SELECT SUM(`price`) FROM `payment_paymentpoint` WHERE $where AND `status` = 1 ")['SUM(`price`)']);?></strong>
...
<script>
CKEDITOR.replace("paymentpoint_notice");
</script>
