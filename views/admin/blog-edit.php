...
if (isset($_POST['Save'])) {
    ...
    if ($CMSNT->get_row("SELECT * FROM `posts` WHERE `title` = '".check_string($_POST['title'])."' AND `id` != ".$row['id']." ")) {
        die('<script type="text/javascript">if(!alert("'.__('Tiêu đề bài viết đã tồn tại trong hệ thống.').'")){window.history.back().location.reload();}</script>');
    }
    ...
    if ($isInsert) {
        $CMSNT->insert("logs", [
            ...
            'device'        => getUserAgent(),
            'createdate'    => gettime(),
            'action'        => __('Chỉnh sửa bài viết')." (".$row['title']." ID ".$row['id'].")."
        ]);
        /** NOTE ACTION */
                    ...
                    <div class="card-body">
                        <form action="" method="POST" enctype="multipart/form-data">
                            <div class="row mb-4">
                                <div class="col-sm-12">
                                    <div class="mb-4">
                                        <label class="form-label" for="name"><?=__('Tiêu đề bài viết:');?></label>
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <div class="input-group">
                                                ...
                                                <input type="text" class="form-control" value="<?=$row['slug'];?>" name="slug" required>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="mb-4">
                                        <label class="form-label" for="code"><?=__('Ảnh nổi bật:');?></label>
                                        <input type="file" class="custom-file-input mb-2" name="image">
                                        <img src="<?=base_url($row['image']);?>" width="200px">
                                    </div>
                                    <div class="mb-4">
                                        <label class="form-label"
                                            for="symbol_left"><?=__('Nội dung bài viết:');?></label>
                                        <textarea id="content"
