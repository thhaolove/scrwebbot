...
$body = [
    'title' => __('Nạp tiền bằng ví MOMO').' | '.$CMSNT->site('title'),
...
if(!empty($_GET['description'])){
    $description = validate_string($_GET['description'], 200, 2);
...
$sql = "SELECT * FROM `payment_momo` WHERE $where_clause ORDER BY `id` DESC LIMIT ?, ?";
$params_with_limit = array_merge($where_params, [$from, $limit]);

$listDatatable = $CMSNT->get_list_safe($sql, $params_with_limit);

$count_sql = "SELECT * FROM `payment_momo` WHERE $where_clause ORDER BY id DESC";
$totalDatatable = $CMSNT->num_rows_safe($count_sql, $where_params);

$urlDatatable = pagination_client(base_url("?action=recharge-momo&limit=$limit&shortByDate=$shortByDate&time=$time&tid=$tid&description=$description&"), $from, $totalDatatable, $limit);
?>
        ...
        <div class="row">
            ...
            <div class="col-lg-7">
                ...
                <div class="account-card p-3">
                    <?=$CMSNT->site('momo_notice');?>
                </div>
            </div>
            <div class="col-lg-5">
                <div class="account-card">
                    <center class="py-3">
                        <h3><?=__('Nạp tiền bằng ví MOMO');?></h3>
                        <?php if($CMSNT->site('momo_number') != ''):?>
                        <?=file_get_contents("https://api.web2m.com/api/qrmomo.php?amount=10000&phone=".$CMSNT->site('momo_number')."&noidung=".urlencode($CMSNT->site('prefix_autobank')).$getUser['id']."&size=300");?>
                        <?php endif?>
                    </center>
                    <ul class="list-group">
                        <li class="list-group-item"><?=__('Số điện thoại:');?> <b id="copySTK"
                                style="color: green;"><?=$CMSNT->site('momo_number');?></b> <button onclick="copy()"
                                class="copy" data-clipboard-target="#copySTK"><i
                        ...
                        <li class="list-group-item" style="font-size:17px;"><?=__('Nội dung chuyển khoản:');?> <b
                            ...
                            <button onclick="copy()" class="copy"
                                data-clipboard-target="#copyNoiDung"><i
                                    class="fas fa-copy"></i></button>

                        </li>
                        <li class="list-group-item"><?=__('Chủ tài khoản:');?>
                            <b><?=$CMSNT->site('momo_name');?></b>
                        </li>
                    </ul>
                ...
                <div class="account-card pt-3">
                    <form action="" method="GET" class="mb-3">
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg col-md-4 col-6">
                                <a href="<?=base_url('?action=recharge-momo');?>" class="shop-widget-btn mb-2"><i
                                ...
                                <tr>
                                    <td colspan="7">
                                        <div class="float-right">
                                            <?=__('Đã thanh toán:');?>
                                            <strong
                                                style="color:red;"><?=format_currency($CMSNT->get_row_safe(" SELECT SUM(`amount`) FROM `payment_momo` WHERE $where_clause ", $where_params)['SUM(`amount`)']);?></strong>
                                            |

                                            <?=__('Thực nhận:');?>
                                            <strong
                                                style="color:blue;"><?=format_currency($CMSNT->get_row_safe(" SELECT SUM(`received`) FROM `payment_momo` WHERE $where_clause ", $where_params)['SUM(`received`)']);?></strong>
                                    </td>
                                </tr>
                            </tfoot>
                        </table>
