...
$body = [
    'title' => __('Recharge PocketFi').' | '.$CMSNT->site('title'),
...
$sql = "SELECT * FROM `payment_pocketfi` WHERE $where_clause ORDER BY `id` DESC LIMIT ?, ?";
$params_with_limit = array_merge($where_params, [$from, $limit]);

$listDatatable = $CMSNT->get_list_safe($sql, $params_with_limit);

$count_sql = "SELECT * FROM `payment_pocketfi` WHERE $where_clause ORDER BY id DESC";
$totalDatatable = $CMSNT->num_rows_safe($count_sql, $where_params);

$urlDatatable = pagination(base_url("?action=recharge-pocketfi&limit=$limit&shortByDate=$shortByDate&time=$time&trans_id=$trans_id&amount=$amount&"), $from, $totalDatatable, $limit);
        ...
        <div class="row">
            <div class="col-md-7">
                <div class="account-card">
                    <h4 class="account-title"><?=__('Recharge PocketFi');?></h4>
                    <div class="text-center mb-4">
                        <img width="300px" src="<?=base_url('mod/img/icon-pocketfi.webp');?>" />
                    </div>
                    <div class="row mb-3">
                        <label class="col-sm-4 col-form-label"
                            for="example-hf-email"><?=__('Enter the deposit amount: ('.$CMSNT->site('pocketfi_currency_code').')');?></label>
                        <div class="col-sm-8">
            ...
            <div class="col-md-5">
                <div class="account-card">
                    ...
                    <?=$CMSNT->site('pocketfi_notice');?>
                </div>
            </div>
            <div class="col-md-12">
                <div class="account-card">
                    <h4 class="account-title"><?=__('Lịch sử nạp PocketFi');?></h4>
                    <form action="<?=base_url();?>" method="GET">
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg col-md-4 col-6">
                                <a href="<?=base_url('?action=recharge-pocketfi');?>" class="shop-widget-btn mb-2"><i
                                ...
                                <tr>
                                    <td colspan="7">
                                        <div class="float-right">
                                            <?=__('Paid:');?>
                                            <strong
                                                style="color:red;"><?=format_currency($CMSNT->get_row_safe(" SELECT SUM(`price`) FROM `payment_pocketfi` WHERE $where_clause AND `status` = 1 ", $where_params)['SUM(`price`)']);?></strong>
...
function loadData() {
    $.ajax({
        ...
        dataType: "JSON",
        data: {
            action: 'notication_topup_pocketfi',
            token: '<?=$getUser['token'];?>'
        },
        success: function(respone) {
            // Nếu thành công
