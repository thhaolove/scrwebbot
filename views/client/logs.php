...
// ✅ An toàn cho IP filtering
if(!empty($_GET['ip'])){
    $ip = validate_ip($_GET['ip']);
    if($ip !== false) {
...
// ✅ An toàn cho content filtering
if(!empty($_GET['content'])){
    $content = validate_string($_GET['content'], 255, 2);
    if($content !== false) {
        $where_conditions[] = '`action` LIKE ?';
        $where_params[] = '%'.$content.'%';
    }
}
...
$sql_list = "SELECT * FROM `logs` WHERE $where_clause ORDER BY `id` DESC LIMIT ?, ?";
$params_with_limit = array_merge($where_params, [$from, $limit]);
$listDatatable = $CMSNT->get_list_safe($sql_list, $params_with_limit);

$sql_count = "SELECT COUNT(*) as total FROM `logs` WHERE $where_clause";
        ...
        <div class="row content-reverse">
            ...
            <div class="col-lg-9">
                <div class="account-card">
                    <h4 class="account-title"><?=__('Nhật ký hoạt động');?></h4>
                    <form action="" method="GET">
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg col-md-4 col-6">
                                <input class="form-control mb-2" value="<?=$ip;?>" name="ip"
                            ...
                            <div class="col-lg col-md-4 col-6">
                                <a href="<?=base_url('?action=logs');?>" class="shop-widget-btn mb-2"><i
