...
$body = [
    'title' => __('Recharge Squadco').' | '.$CMSNT->site('title'),
...
// Validate transaction_ref
if(!empty($_GET['transaction_ref'])){
    $transaction_ref = validate_alphanumeric($_GET['transaction_ref'], 100);
    if($transaction_ref !== false) {
        $where_conditions[] = '`transaction_ref` = ?';
        $where_params[] = $transaction_ref;
    }
}
...
if(!empty($_GET['billName'])){
    $billName = validate_string($_GET['billName'], 200, 2);
    if($billName !== false) {
...
$sql = "SELECT * FROM `payment_squadco` WHERE $where_clause ORDER BY `id` DESC LIMIT ?, ?";
$params_with_limit = array_merge($where_params, [$from, $limit]);

$listDatatable = $CMSNT->get_list_safe($sql, $params_with_limit);

$count_sql = "SELECT * FROM `payment_squadco` WHERE $where_clause ORDER BY id DESC";
$totalDatatable = $CMSNT->num_rows_safe($count_sql, $where_params);

$urlDatatable = pagination(base_url("?action=recharge-quadco&limit=$limit&shortByDate=$shortByDate&time=$time&transaction_ref=$transaction_ref&amount=$amount&"), $from, $totalDatatable, $limit);
        ...
        <div class="row">
            <div class="col-md-7">
                <div class="account-card">
                    <h4 class="account-title"><?=__('Recharge Squadco');?></h4>
                    <div class="text-center mb-4">
                        <img width="300px" src="<?=base_url('assets/img/logo-squadco.svg');?>" />
                    </div>
                    <div class="row mb-3">
                        <label class="col-sm-4 col-form-label"
                            for="example-hf-email"><?=__('Enter the deposit amount: ('.$CMSNT->site('squadco_currency_code').')');?></label>
                        <div class="col-sm-8">
                    ...
                    <center>
                        <div class="wallet-form">
                            <button type="button" onclick="SquadPay()" id="btnSubmit"><?=__('Submit');?></button>
                        </div>
                    </center>
                </div>
            </div>
            ...
            <div class="col-md-12">
                <div class="account-card">
                    <h4 class="account-title"><?=__('Lịch sử nạp Squadco');?></h4>
                    <form action="<?=base_url();?>" method="GET">
                        <input type="hidden" name="action" value="recharge-squadco">
                        <div class="row">
                            <div class="col-lg col-md-4 col-6">
                                <input class="form-control col-sm-2 mb-1" value="<?=$transaction_ref;?>"
                                    name="transaction_ref" placeholder="<?=__('Search transaction ref');?>">
                            </div>
                            <div class="col-lg col-md-4 col-6">
                            ...
                            <div class="col-lg col-md-4 col-6">
                                <a href="<?=base_url('?action=recharge-squadco');?>" class="shop-widget-btn mb-2"><i
                    ...
                    <div class="table-scroll">
                        <table class="table fs-sm mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="text-center"><b><?=$row['transaction_ref'];?></b></td>
                                ...
                                <tr>
                                    <td colspan="7">
                                        <div class="float-right">
                                            <?=__('Paid:');?>
                                            <strong
                                                style="color:red;"><?=format_currency($CMSNT->get_row_safe(" SELECT SUM(`price`) FROM `payment_squadco` WHERE $where_clause ", $where_params)['SUM(`price`)']);?></strong>
