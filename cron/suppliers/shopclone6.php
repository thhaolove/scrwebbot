<?php
    ...
    /* START CHỐNG SPAM */
    if (time() > $CMSNT->site('time_cron_suppliers_shopclone6')) {
        if (time() - $CMSNT->site('time_cron_suppliers_shopclone6') < 5) {
            die('Thao tác quá nhanh, vui lòng thử lại sau!');
        }
    }
    $CMSNT->update("settings", [
        'value' => time()
    ], " `name` = 'time_cron_suppliers_shopclone6' ");
    ...
    foreach($CMSNT->get_list_safe(" SELECT * FROM `suppliers` WHERE `status` = ? AND `type` = ? ", [1, 'SHOPCLONE6']) as $supplier){
        ...
        // CURL LẤY SẢN PHẨM
        $result = listProduct_API_SHOPCLONE6($supplier['domain'], $supplier['username'], $supplier['password'], $supplier['proxy']);
        ...
        if($result['status'] == 'success'){
            foreach($result['categories'] as $category){
                ...
                foreach($category['accounts'] as $api){
