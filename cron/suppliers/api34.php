...
/* START CHỐNG SPAM */
if (time() > $CMSNT->site('time_cron_suppliers_api34')) {
    if (time() - $CMSNT->site('time_cron_suppliers_api34') < 5) {
        die('Thao tác quá nhanh, vui lòng thử lại sau!');
    }
}
$CMSNT->update("settings", [
    'value' => time()
], " `name` = 'time_cron_suppliers_api34' ");



foreach ($CMSNT->get_list_safe(" SELECT * FROM `suppliers` WHERE `status` = ? AND `type` = ? ", [1, 'API_34']) as $supplier) {
    // CẬP NHẬT SỐ DƯ API
    if (!empty($supplier['api_key'])) {
        $result1 = balance_API_34($supplier['domain'], $supplier['api_key'], $supplier['proxy']);
    ...
    // CURL LẤY SẢN PHẨM
    $result = listProduct_API_34($supplier['domain'], $supplier['api_key'], $supplier['proxy']);
    ...
    if (isset($result) && isset($result['success']) && $result['success'] == true) {
        ...
        foreach ($result['data'] as $api) {
            ...
            if (!empty($api['description'])) {
                $desc_parts[] = $api['description'];
            }
            if (!empty($api['liveTime'])) {
                $desc_parts[] = 'Live: ' . $api['liveTime'];
            }
            if (!empty($api['country'])) {
                $desc_parts[] = 'Country: ' . $api['country'];
            }
            $api_desc = implode(' | ', $desc_parts);
            $api_desc = $supplier['check_string_api'] == 'OFF' ? $api_desc : check_string($api_desc);

            $api_stock = isset($api['quantity']) ? intval($api['quantity']) : 0;
            $api_price = check_string($api['price']); // Giá gốc
