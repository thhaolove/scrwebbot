<?php
    ...
    /* START CHỐNG SPAM */
    if (time() > $CMSNT->site('time_cron_suppliers_api6')) {
        if (time() - $CMSNT->site('time_cron_suppliers_api6') < 5) {
            die('Thao tác quá nhanh, vui lòng thử lại sau!');
        }
    }
    $CMSNT->update("settings", [
        'value' => time()
    ], " `name` = 'time_cron_suppliers_api6' ");



    foreach($CMSNT->get_list_safe(" SELECT * FROM `suppliers` WHERE `status` = ? AND `type` = ? ", [1, 'API_6']) as $supplier){
        // CẬP NHẬT SỐ DƯ API
        $result1 = balance_API_6($supplier['domain'], $supplier['api_key']);
        $data = json_decode($result1, true);
        if(isset($data['balance'])){
            $CMSNT->update('suppliers', [
                'price' => format_currency($data['balance']),
        ...
        // CURL LẤY SẢN PHẨM
        $result = curl_get($supplier['domain'].'/api.php?apikey='.$supplier['api_key'].'&action=get-services');
        $result = json_decode($result, true);
        if($result['code'] == 0){
            foreach($result['data'] as $api){

                $api_id = check_string($api['service_id']);
                $api_name = check_string($api['service_name']);
