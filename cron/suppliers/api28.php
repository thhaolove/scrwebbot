<?php
    ...
    /* START CHỐNG SPAM */
    if (time() > $CMSNT->site('time_cron_suppliers_api28')) {
        if (time() - $CMSNT->site('time_cron_suppliers_api28') < 2) {
            die('Thao tác quá nhanh, vui lòng thử lại sau!');
        }
    }
    $CMSNT->update("settings", [
        'value' => time()
    ], " `name` = 'time_cron_suppliers_api28' ");



    foreach($CMSNT->get_list_safe(" SELECT * FROM `suppliers` WHERE `status` = ? AND `type` = ? ", [1, 'API_28']) as $supplier){
        // CẬP NHẬT SỐ DƯ API VÀ TOKEN
        $data = balance_API_28($supplier['domain'], $supplier['username'], $supplier['password']);
        $data = json_decode($data, true);
        $CMSNT->update('suppliers', [
            'price' => isset($data['amount']) ? $data['amount'] : $data['message'],
            'token' => isset($data['token']) ? $data['token'] : $supplier['token']
        ], " `id` = '".$supplier['id']."' ");

        // CURL LẤY SẢN PHẨM
        $result = listProduct_API_28($supplier['domain']);
        $result = json_decode($result, true);
        if($result['status'] == 'success'){
            foreach($result['categories'] as $category){
                if(!empty($category['account'])){
                    foreach($category['account'] as $api){
                        $api_id = check_string($api['id']);
                        $api_name = $supplier['check_string_api'] == 'OFF' ? $api['name'] : check_string($api['name']);
                        $api_stock = filter_var($api['quantity'], FILTER_SANITIZE_NUMBER_INT);
                        ...
                        if(!$product = $CMSNT->get_row_safe(" SELECT * FROM `products` WHERE `api_id` = ? AND `supplier_id` = ? ", [$api_id, $supplier['id']])){
                            ...
                            $CMSNT->insert('products', [
                                ...
                                'category_id'       => 0, // Sử dụng ID chuyên mục
