<?php
    ...
    /* START CHỐNG SPAM */
    if (time() > $CMSNT->site('time_cron_suppliers_api29')) {
        if (time() - $CMSNT->site('time_cron_suppliers_api29') < 2) {
            die('Thao tác quá nhanh, vui lòng thử lại sau!');
        }
    }
    $CMSNT->update("settings", [
        'value' => time()
    ], " `name` = 'time_cron_suppliers_api29' ");
    ...
    foreach($CMSNT->get_list_safe(" SELECT * FROM `suppliers` WHERE `status` = ? AND `type` = ? ", [1, 'API_29']) as $supplier){
        // CẬP NHẬT SỐ DƯ API
        $result = balance_API_29($supplier['domain'], $supplier['api_key']);
        ...
        foreach($mailType as $mail){
            $result = listProduct_API_29($supplier['domain'], $mail);
            $result = json_decode($result, true);
            if(!isset($result['data'])){
                continue;
            }
            $api_id = $mail;
            $api_name = $mail;
            $api_stock = intval(check_string($result['data']));
            $api_desc = NULL;
