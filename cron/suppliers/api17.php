<?php
    ...
    /* START CHỐNG SPAM */
    if (time() > $CMSNT->site('time_cron_suppliers_api17')) {
        if (time() - $CMSNT->site('time_cron_suppliers_api17') < 2) {
            die('Thao tác quá nhanh, vui lòng thử lại sau!');
        }
    }
    $CMSNT->update("settings", [
        'value' => time()
    ], " `name` = 'time_cron_suppliers_api17' ");



    foreach($CMSNT->get_list_safe(" SELECT * FROM `suppliers` WHERE `status` = ? AND `type` = ? ", [1, 'API_17']) as $supplier){
        // CẬP NHẬT SỐ DƯ API
        $getPrice = balance_API_17($supplier['domain'], $supplier['username'], $supplier['password']);
        $CMSNT->update('suppliers', [
            'price' => check_string($getPrice),
            'update_gettime'    => gettime()
        ], " `id` = ? ", [$supplier['id']]);

        // CURL LẤY SẢN PHẨM
        $result = listProduct_API_17($supplier['domain'], $supplier['username'], $supplier['password']);
        $result = json_decode($result, true);
        if($result['status'] == 'success'){
            foreach($result['data'] as $api){
                $api_id = check_string($api['id']);
                $api_name = $supplier['check_string_api'] == 'OFF' ? $api['title'] : check_string($api['title']);
                $api_stock = intval(check_string($api['amount']));
                $api_desc = $supplier['check_string_api'] == 'OFF' ? $api['note'] : check_string($api['note']);
                $api_price = intval(check_string($api['money']));//
