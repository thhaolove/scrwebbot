<?php
    ...
    /* START CHỐNG SPAM */
    if (time() > $CMSNT->site('time_cron_suppliers_api4')) {
        if (time() - $CMSNT->site('time_cron_suppliers_api4') < 5) {
            die('Thao tác quá nhanh, vui lòng thử lại sau!');
        }
    }
    $CMSNT->update("settings", [
        'value' => time()
    ], " `name` = 'time_cron_suppliers_api4' ");



    foreach($CMSNT->get_list_safe(" SELECT * FROM `suppliers` WHERE `status` = ? AND `type` = ? ", [1, 'API_4']) as $supplier){

        // CẬP NHẬT SỐ DƯ API
        $result = balance_API_4($supplier['domain'], $supplier['username'], $supplier['password']);
        $result = json_decode($result, true);
        if(isset($result['data']['Data']['userDetail']['coin'])){
            $CMSNT->update('suppliers', [
                'price' => format_currency($result['data']['Data']['userDetail']['coin']),
                'token' => $result['data']['Data']['accessToken']
            ], " `id` = '".$supplier['id']."' ");
        }

        // CURL LẤY SẢN PHẨM
        $result = listProduct_API_4($supplier['domain']);
        $result = json_decode($result, true);
        if(isset($result['data'])){
            foreach($result['data'] as $api){
                ...
                $api_stock = intval(check_string($api['totalProduct']));
