<?php
    ...
    /* START CHỐNG SPAM */
    if (time() > $CMSNT->site('time_cron_suppliers_api22')) {
        if (time() - $CMSNT->site('time_cron_suppliers_api22') < 5) {
            die('Thao tác quá nhanh, vui lòng thử lại sau!');
        }
    }
    $CMSNT->update("settings", [
        'value' => time()
    ], " `name` = 'time_cron_suppliers_api22' ");



    foreach($CMSNT->get_list_safe(" SELECT * FROM `suppliers` WHERE `status` = ? AND `type` = ? ", [1, 'API_22']) as $supplier){
  
 
  
        // CURL LẤY SẢN PHẨM
        $result = listProduct_API_22($supplier['domain'], $supplier['token']);
        ...
        if(isset($result['data'])){
            foreach($result['data'] as $api){
                ...
                $api_price = floatval(check_string($api['price']));
                $ck = $api_price * $supplier['discount'] / 100;
                $price = floatval(check_string($api['price']));
