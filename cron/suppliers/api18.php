<?php
    ...
    /* START CHỐNG SPAM */
    if (time() > $CMSNT->site('time_cron_suppliers_api18')) {
        if (time() - $CMSNT->site('time_cron_suppliers_api18') < 2) {
            die('Thao tác quá nhanh, vui lòng thử lại sau!');
        }
    }
    $CMSNT->update("settings", [
        'value' => time()
    ], " `name` = 'time_cron_suppliers_api18' ");



    foreach($CMSNT->get_list_safe(" SELECT * FROM `suppliers` WHERE `status` = ? AND `type` = ? ", [1, 'API_18']) as $supplier){
        // CẬP NHẬT SỐ DƯ API
        $result = balance_API_18($supplier['domain'], $supplier['api_key']);
        ...
        // CURL LẤY SẢN PHẨM
        $result = listProduct_API_18($supplier['domain']);
        $result = json_decode($result, true);
        if(isset($result['Data'])){
            foreach($result['Data'] as $api){
                $api_id = check_string($api['MailCode']);
                $api_name = check_string($api['MailCode']);
                $api_stock = intval(check_string($api['Instock']));
                $api_desc = NULL;
                $api_price = intval(check_string($api['PriceUsd'] * 25000));//
                $ck = $api_price * $supplier['discount'] / 100;
                $price = intval(check_string($api['PriceUsd'] * 25000));
