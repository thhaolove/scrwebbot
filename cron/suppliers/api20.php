<?php
    ...
    /* START CHỐNG SPAM */
    if (time() > $CMSNT->site('time_cron_suppliers_api20')) {
        if (time() - $CMSNT->site('time_cron_suppliers_api20') < 5) {
            die('Thao tác quá nhanh, vui lòng thử lại sau!');
        }
    }
    $CMSNT->update("settings", [
        'value' => time()
    ], " `name` = 'time_cron_suppliers_api20' ");



    foreach($CMSNT->get_list_safe(" SELECT * FROM `suppliers` WHERE `status` = ? AND `type` = ? ", [1, 'API_20']) as $supplier){
        ...
        $api_stock = intval(check_string($api['stock']));
        $api_desc = NULL;
