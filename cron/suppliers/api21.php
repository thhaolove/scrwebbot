<?php
    ...
    /* START CHỐNG SPAM */
    if (time() > $CMSNT->site('time_cron_suppliers_api21')) {
        if (time() - $CMSNT->site('time_cron_suppliers_api21') < 5) {
            die('Thao tác quá nhanh, vui lòng thử lại sau!');
        }
    }
    $CMSNT->update("settings", [
        'value' => time()
    ], " `name` = 'time_cron_suppliers_api21' ");



    foreach($CMSNT->get_list_safe(" SELECT * FROM `suppliers` WHERE `status` = ? AND `type` = ? ", [1, 'API_21']) as $supplier){
  
 
  
        // CURL LẤY SẢN PHẨM
        $result = listProduct_API_21($supplier['domain'], $supplier['token']);
