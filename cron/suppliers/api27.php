<?php
    ...
    /* START CHỐNG SPAM */
    if (time() > $CMSNT->site('time_cron_suppliers_api27')) {
        if (time() - $CMSNT->site('time_cron_suppliers_api27') < 2) {
    ...
    foreach($CMSNT->get_list_safe(" SELECT * FROM `suppliers` WHERE `status` = ? AND `type` = ? ", [1, 'API_27']) as $supplier){
