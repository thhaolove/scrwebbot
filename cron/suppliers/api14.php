<?php
    ...
    /* START CHỐNG SPAM */
    if (time() > $CMSNT->site('time_cron_suppliers_api14')) {
        if (time() - $CMSNT->site('time_cron_suppliers_api14') < 5) {
            die('Thao tác quá nhanh, vui lòng thử lại sau!');
        }
    }
    $CMSNT->update("settings", [
        'value' => time()
    ], " `name` = 'time_cron_suppliers_api14' ");



    foreach($CMSNT->get_list_safe(" SELECT * FROM `suppliers` WHERE `status` = ? AND `type` = ? ", [1, 'API_14']) as $supplier){
        // CẬP NHẬT SỐ DƯ API
        $result1 = balance_API_14($supplier['domain'], $supplier['token']);
        ...
        // CURL LẤY SẢN PHẨM
        $result = listProduct_API_14($supplier['domain'], $supplier['token']);
        $result = json_decode($result, true);
        if(isset($result['products'])){
            foreach($result['products'] as $api){
