<?php
    ...
    /* START CHỐNG SPAM */
    if (time() > $CMSNT->site('time_cron_suppliers_api19')) {
        if (time() - $CMSNT->site('time_cron_suppliers_api19') < 5) {
            die('Thao tác quá nhanh, vui lòng thử lại sau!');
        }
    }
    $CMSNT->update("settings", [
        'value' => time()
    ], " `name` = 'time_cron_suppliers_api19' ");



    foreach($CMSNT->get_list_safe(" SELECT * FROM `suppliers` WHERE `status` = ? AND `type` = ? ", [1, 'API_19']) as $supplier){
        // CẬP NHẬT SỐ DƯ API
        $result = balance_API_19($supplier['domain'], $supplier['api_key']);
        ...
        // CURL LẤY SẢN PHẨM
        $result = listProduct_API_19($supplier['domain'], $supplier['api_key']);
        $result = json_decode($result, true);
        if(isset($result['data'])){
            foreach($result['data'] as $api){
                $api_id = check_string($api['id']);
                $api_name = check_string($api['name']);
                $api_stock = intval(check_string($api['quality']));
                $api_desc = NULL;
