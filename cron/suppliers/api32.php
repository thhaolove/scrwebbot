<?php
    ...
    /* START CHỐNG SPAM */
    if (time() > $CMSNT->site('time_cron_suppliers_api32')) {
        if (time() - $CMSNT->site('time_cron_suppliers_api32') < 5) {
            die('Thao tác quá nhanh, vui lòng thử lại sau!');
        }
    }
    $CMSNT->update("settings", [
        'value' => time()
    ], " `name` = 'time_cron_suppliers_api32' ");



    foreach($CMSNT->get_list_safe(" SELECT * FROM `suppliers` WHERE `status` = ? AND `type` = ? ", [1, 'API_32']) as $supplier){
        // CẬP NHẬT SỐ DƯ API
        $result1 = balance_API_32($supplier['domain'], $supplier['api_key'], $supplier['proxy']);
        $result = json_decode($result1, true);
        if(isset($result['success']) && $result['success'] == true){
            $CMSNT->update('suppliers', [
                'price' => format_currency(check_string($result['data']['balance'])),
        ...
        // CURL LẤY SẢN PHẨM
        $result = listProduct_API_32($supplier['domain'], $supplier['api_key'], $supplier['proxy']);
        $result = json_decode($result, true);
        if($result['success'] == true){
            $category_id = 0; // Mặc định ID chuyên mục sẽ không có
            foreach($result['listproduct'] as $api){

                $api_id = check_string($api['id']);
                $api_name = $supplier['check_string_api'] == 'OFF' ? $api['name'] : check_string($api['name']);
                $api_desc = $supplier['check_string_api'] == 'OFF' ? $api['description'] : '';
                $api_stock = intval(check_string(getStock_API_32($supplier['domain'], $supplier['api_key'], $api_id, $supplier['proxy'])));
                ...
                if(!$product = $CMSNT->get_row_safe(" SELECT * FROM `products` WHERE `api_id` = ? AND `supplier_id` = ? ", [$api_id, $supplier['id']])){
                    ...
                    $api_name = $supplier['check_string_api'] == 'OFF' ? $api['name'] : check_string($api['name']);
                    $api_desc = $supplier['check_string_api'] == 'OFF' ? $api['description'] : '';
                    $api_stock = intval(check_string(getStock_API_32($supplier['domain'], $supplier['api_key'], $api['id'], $supplier['proxy'])));//
                    $api_price = check_string($api['price']);//
                    $ck = $api_price * $supplier['discount'] / 100;
