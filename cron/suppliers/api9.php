<?php
    ...
    /* START CHỐNG SPAM */
    if (time() > $CMSNT->site('time_cron_suppliers_api9')) {
        if (time() - $CMSNT->site('time_cron_suppliers_api9') < 5) {
            die('Thao tác quá nhanh, vui lòng thử lại sau!');
        }
    }
    $CMSNT->update("settings", [
        'value' => time()
    ], " `name` = 'time_cron_suppliers_api9' ");



    foreach($CMSNT->get_list_safe(" SELECT * FROM `suppliers` WHERE `status` = ? AND `type` = ? ", [1, 'API_9']) as $supplier){
        // CẬP NHẬT SỐ DƯ API
        $data = balance_API_9($supplier['domain'], $supplier['api_key']);
        $data = json_decode($data, true);
        if($data['error'] == 0){
            $CMSNT->update('suppliers', [
                'price' => format_currency(check_string($data['data']['balance']))
            ], " `id` = '".$supplier['id']."' ");
        }
        ...
        if($result['error'] == 0){
            foreach($result['data'] as $category){
                foreach($category['products'] as $api){
                    ...
                    $api_stock = intval(check_string($api['remain']));
